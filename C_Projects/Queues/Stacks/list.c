...
/*
 * This function should indicate whether a given list is currently empty.
 * Specifically, it should return 1 if the specified list is empty (i.e.
 * contains no elements) and 0 otherwise.
 *
 * Params:
 *   list - the list whose emptiness is being questioned.  May not be NULL.
 */
...
/*
 * This function should return the value at the front of the list.
 * Specifically, it should return the value of the head node
 *
 * Params:
 *   list - May not be NULL.
 */
void* first(struct list* list) {
  return list->head->val;
}

/*
 * This function should remove the head of the list and make head->next
...
void remove_head(struct list* list) {
  struct link* cur = list->head->next;
  free(list->head);
  list->head = cur;
}