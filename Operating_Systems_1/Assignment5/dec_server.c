...
/*
 * This function should decrypt the plaintext string using the provided key
 *
 * Params:
 *   pain_text - the string to decrypt.
 *   key - the key used for decryption.
 */
void decrypt_msg(char *plain_text, char *key) {
    //needed variables
    int i, n, k;
    int new_letter;
    ...
    for(i = 0; i < strlen(plain_text); i++) {
        ...
        //add the numbers (plus 27 for out of bounds) and modulo
        //then add the corrsponding letter
        new_letter = ((n - k) + 27) % 27;
    ...
    while(1) {
        ...
        switch(spawn_pid) {
            ...
            //create a buffer and clear it
                ...
                if(strcmp(bf, "dec_client") == 0) {
                    chars_written = send(connection_sock, "This is dec_server", 15, 0);
                    if (chars_written < 0){
